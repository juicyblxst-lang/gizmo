...
// Factual visualization data. It reuses the engine's own 1H prices and lead-lag math.
app.get('/api/tools/leadlag-chart',async(req,res)=>{
    try{
        ...
        if(!btcPrices) return res.status(503).json({error:'BTC market data unavailable'});
        if(requested==='BTC-USDT-SWAP'){
            const candidates=PAIRS.filter(p=>p!=='BTC-USDT-SWAP'&&allPrices[p]);
            const analyses=candidates.map(p=>analyzeLeadLagPair(p,allPrices[p],btcPrices,80)).filter(a=>a.available);
            if(!analyses.length) return res.status(503).json({error:'No measurable BTC lead-lag relationship available'});
            analyses.sort((a,b)=>Math.abs(b.correlation)-Math.abs(a.correlation));
            return res.json({...analyses[0],selectedFor:'BTC market view',alternatives:analyses.map(a=>({follower:a.follower,lagHours:a.lagHours,correlation:a.correlation}))});
        }
        if(!allPrices[requested]) return res.status(503).json({error:'Follower market data unavailable'});
        res.json(analyzeLeadLagPair(requested,allPrices[requested],btcPrices,80));
    }catch(error){res.status(500).json({error:error.message});}
});

// Workspace intelligence bridge. Existing tool endpoints and UI remain unchanged.
